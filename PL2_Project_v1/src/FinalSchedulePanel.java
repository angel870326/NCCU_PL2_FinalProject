...
public class FinalSchedulePanel extends JPanel{
	...
	private static final Color otherCourse = Color.decode("#355C7D");  // #CDFDFD
	private static final Color scheduledCourse = Color.decode("#F67280");  //#7C5DB3
	private static final Color selectCourse = Color.decode("#F8B195");  // #E3FEFE
	private static final Color mainColor = Color.decode("#A392C7"); //#512350 
	private static final Color planToTake = Color.decode("#E8175D");
...
//		filterLectureList=new ArrayList<ArrayList<Lecture>>();
		...
		monPanel.setBackground(Color.decode("#6C5B7B"));
		tuePanel.setBackground(Color.decode("#6C5B7B"));
		wedPanel.setBackground(Color.decode("#6C5B7B"));
		thuPanel.setBackground(Color.decode("#6C5B7B"));
		friPanel.setBackground(Color.decode("#6C5B7B"));
...
//		setColor(eastPanel, "green");
		mainPanel.setBackground(Color.decode("#355C7D"));
		westPanel.setBackground(Color.decode("#6C5B7B"));
...
//		mainPanel.add(indDayOfWeekPanel("五"));
		...
		btnPanel.setBackground(Color.decode("#DA7B93"));
		sendEmailPanel.setBackground(Color.decode("#E7717D"));
		rightDownPanel.setBackground(Color.decode("#547980"));
		eastPanel.add(rightDownPanel, BorderLayout.SOUTH);
		add(mainPanel,BorderLayout.WEST);

		add(eastPanel, BorderLayout.EAST);

		setBackground(Color.decode("#594F4F"));//grey #778899
	...
	public void getPlanToTakeToRegister()  throws SQLException
	{
		...
		try {
			...
			while(result.next()) {
				...
				register.addCourse(lectureId,lectureName,Double.parseDouble(lectureCredits),lectureDay,lectureTime,lectureClassroom,type);
...
//			System.out.println("bk,");
			while(result.next()) {
				...
				register.addCourse(lectureId,lectureName,lectureCredits,lectureDay,lectureTime,lectureClassroom,type);
...
//		System.out.println("Size: "+selectCoursePanel.getFilterLectureList().get(lectureListCount).size());
		...
		for (int k = 0; k<filterLectureList.get(lectureListCount).size();k++) {
			...
			register.addCourse(lec.getLectureID(),lec.getLectureName(),Double.parseDouble(lec.getLecturePoint()),lec.getLectureTime().substring(0,1),lec.getLectureTime().substring(1),lec.getLectureClassRoom(),type);
		}
		...
		String dayOfWeek= day;
			...
			for (String timeOfDay : timeList)
			{
				if (count==0)
				{
					...
					{
						if (course.getDay().contains(dayOfWeek) && course.getTime().contains(abbTimeOfDay))
								{
									flag = true;
...
//				add(westPanel,BorderLayout.WEST);
	...
	public JPanel createCreditTextArea()
	{
		...
		subPanel.setBackground(Color.decode("#547980"));
		return subPanel;
	...
	public JPanel createCourseDetailTextArea()
	{
		...
		courseInfoArea.setBackground(Color.decode("#547980"));
	...
	public String courseInfoToString()
	{
		...
		{
			ans+="CourseID: "+cou.getCourseID()+", CourseName: "+cou.getCourseName()+", Credits: "+cou.getCredits()+", Day: "+cou.getDay()+", Time:"+cou.getTime()+", Classroom: "+cou.getClassroom()+"\n";
		}
		return ans;
	}
	...
	public void arrange(JTextArea textArea, String type) {
		...
		{
			textArea.setBorder(BorderFactory.createLineBorder(Color.black, 1, true));
			textArea.setBackground(planToTake);
			textArea.setForeground(Color.white);
		}
	}
	...
	public void screenshot() throws AWTException, IOException
	{
    	...
    	ImageIO.write(screenShot, "JPG", new File("finalScheduleScreenshot.jpg"));
	}
...
//			set subject line
			...
			pdfAttachment.attachFile("/Users/bensonhermes/java_3/final/finalScheduleScreenshot.jpg");
